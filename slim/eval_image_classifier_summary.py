...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.csv_name is None:
        csv_path = os.path.join(FLAGS.eval_dir,strftime("%Y_%m_%d_%H_%M_%S", gmtime())+"_"+FLAGS.dataset_split_name + "_prediciton.csv")
    else:
    ...
    with open(csv_path,"w+") as f:
        ...
        for i in range(len(pi_name)):
            d = {'src':pi_name[i],'label':pi_label[i],'predict':pi_prediction[i]}
            writer.writerow(d)
